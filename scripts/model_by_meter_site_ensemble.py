...
for _,row in uid_partition.iterrows():
    ...
    tic = time.time()
    fcaster_base = model_class(**model_kwargs)
    fcaster_base.prepare_features(train_data = train_data_)
    fcaster_base.train_features = reduce_mem_usage(fcaster_base.train_features)
    tac = time.time()
    print(f"[INFO] time elapsed preparing the features: {(tac-tic)/60.} min.\n")

    predictions_ = list()
    hyperparams = (pd.read_csv(f"{args.hyperparams_path}/hs_lgb_meter{row.meter}_site{row.site_id}.csv", sep=";")
                   .sort_values("error")
    ...
    for idx in range(TOP_N):
        ...
        best_model_params = literal_eval(hyperparams.loc[idx, "params"])
        best_model_params["num_iterations"] = hyperparams.loc[idx, "best_iteration"]
        model_params = {**default_model_params, **best_model_params}
        model_params["learning_rate"] = 0.01
        fcaster = copy.deepcopy(fcaster_base)
        ...
        predictions_.append(predictions.y_pred.values)
        tac = time.time()
        print(f"[INFO] time elapsed predicting: {(tac-tic)/60.} min.\n")

    # aggregating all the predictions
    predictions = pd.DataFrame({"ds":test_data_.ds,
                                "building_id":test_data_.building_id,
                                "meter":row.meter,
                                "y_pred":trimean(np.asarray(predictions_), axis=0)})
