...
def objective(trial):
    sampled_params["grow_policy"] = trial.suggest_categorical("grow_policy", ["depthwise", "lossguide"])
    if sampled_params["grow_policy"] == "lossguide":
        sampled_params["max_depth"] = 0
        sampled_params["tree_method"] = "hist"
        sampled_params["max_leaves"] = trial.suggest_int("num_leaves", 32, 1024)
        
    else:
        sampled_params["tree_method"] = "exact"
        sampled_params["max_depth"] = trial.suggest_int("max_depth", 5, 10)
                                                 
    _sampled_params = {        
        "min_child_weight":int(trial.suggest_discrete_uniform("min_child_weight", 5, 30, 5)),
        "colsample_bytree":trial.suggest_discrete_uniform("colsample_bytree", 0.5, 1.0, 0.1),
        "lambda":trial.suggest_discrete_uniform("lambda", 0., 5.0, 1.0)
    }
