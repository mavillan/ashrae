...
if os.path.exists(f"./results/hs_xgb_meter{args.meter}_site{args.site}.csv"):
    logger = open(f"./results/hs_xgb_meter{args.meter}_site{args.site}.csv", "a")
else:
    logger = open(f"./results/hs_xgb_meter{args.meter}_site{args.site}.csv", "w")
...
def objective(trial):
    ...
    default_model_params["grow_policy"] = "depthwise"
    default_model_params["tree_method"] = "exact"                                         
    sampled_params = {
        "max_depth":trial.suggest_int("max_depth", 5, 10),
        "min_child_weight":int(trial.suggest_int("min_child_weight", 1, 30)),
    ...
    valid_error = fcaster.model.model.best_score
